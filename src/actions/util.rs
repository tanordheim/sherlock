...
pub fn copy_to_clipboard(string: &str) -> Result<(), SherlockError> {
    let mut ctx = ClipboardContext::new().map_err(|e| sherlock_error!(
...
pub fn clear_cached_files() -> Result<(), SherlockError> {
    let config = CONFIG.get().ok_or_else(|| sherlock_error!(SherlockErrorType::ConfigError(None), ""))?;
    let home = home_dir()?;
    // Clear sherlocks cache
    fs::remove_dir_all(home.join(".cache/sherlock")).map_err(|e| sherlock_error!(
            SherlockErrorType::DirRemoveError(String::from("~/.cache/sherlock")),
            &e.to_string()
    ))?;

    // Clear app cache
    fs::remove_file(&config.behavior.cache).map_err(|e| sherlock_error!(
...
pub fn reset_app_counter() -> Result<(), SherlockError> {
    let home = home_dir()?;
    fs::remove_file(home.join(".sherlock/counts.json")).map_err(|e| sherlock_error!(
...
pub fn parse_default_browser() -> Result<String, SherlockError> {
    ...
    let browser = read_lines(browser_file)
        ...
        .ok_or_else(|| sherlock_error!(
            SherlockErrorType::FileParseError(browser_file.clone()),
            ""
        ))?;
    Ok(browser)
}
